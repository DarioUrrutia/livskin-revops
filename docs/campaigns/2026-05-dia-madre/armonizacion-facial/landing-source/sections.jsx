...
// ============== HERO — TOFU emocional, identidad ==============
const Hero = ({ eyebrow, headline, sub, accent }) => {
  return (
    <section className="hero-section">
      <style>{`
        ...
        @media (min-width: 720px) { .hero-inner { grid-template-columns: 1fr 1fr; gap: 40px; } }
        @media (min-width: 1024px) { .hero-inner { gap: 80px; } }
      `}</style>
      <div className="hero-inner">
        <div className="hero-text" style={{ textAlign: "center", display: "flex", flexDirection: "column", alignItems: "center" }}>
          ...
          <h1 className="display-bold" style={{
            ...
            color: "var(--ink)", letterSpacing: "-0.005em", marginBottom: 22,
          }}>
            {headline.split("\n").map((l, i) => <div key={i}>{l}</div>)}
          </h1>
          <p style={{ fontSize: "clamp(15px, 2.2vw, 17px)", lineHeight: 1.65, color: "var(--ink-soft)", maxWidth: 460, marginBottom: 32 }}>{sub}</p>
          <a href="#proceso" style={{
            display: "inline-flex", alignItems: "center", gap: 10,
            color: "var(--ink)", fontFamily: "Montserrat, sans-serif",
            fontWeight: 600, fontSize: 13, letterSpacing: "0.18em", textTransform: "uppercase",
            borderBottom: "1.5px solid var(--ink)", paddingBottom: 6,
          }}>Descubre más <span aria-hidden="true">→</span></a>
        </div>
...
const Benefits = ({ accent }) => {
  const items = [
    { icon: "heart",   title: "Decide por ti",        desc: "No es para verte distinta. Es para verte como ya eres." },
    { icon: "leaf",    title: "Naturalidad real",     desc: "Tu expresión intacta. Sin marcas, sin exageraciones." },
    ...
    <section id="beneficios" className="sec" style={{ background: "var(--bg-pink)" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto", textAlign: "center" }}>
        ...
        <h2 className="display-bold" style={{ fontSize: "clamp(26px, 5vw, 44px)", fontWeight: 700, color: "var(--ink)", marginBottom: 40, letterSpacing: "-0.005em", lineHeight: 1.15 }}>
          Tu rostro no necesita permiso.
        </h2>
        <div style={{ marginBottom: 56, display: "flex", justifyContent: "center" }}>
          <img src="images/filosofia.jpg" alt="" style={{
            width: "100%", maxWidth: 480, aspectRatio: "1/1", objectFit: "cover",
            borderRadius: 6, display: "block",
          }} />
        </div>
